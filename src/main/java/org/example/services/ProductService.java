...
public class ProductService implements IService<Product> {
    ...
    /**
     * Sous-ensemble pour session invitée / index Elasticsearch {@code guest_admin_catalog} :
     * produits publiés sans {@code user_id} ou dont le propriétaire est un compte administrateur.
     */
    public List<Product> findPublishedCatalogByAdmin() throws SQLException {
        List<Product> pub = findPublishedCatalog();
        if (pub.isEmpty()) {
            return pub;
        }
        ...
        for (Product p : pub) {
            ...
            if (ou.isPresent() && ou.get().getRole() == Role.ADMIN) {
                out.add(p);
            }
        }
        return out;
    ...
    /**
     * Produits les plus vendus (somme des quantités sur {@code ligne_commande}), pour suggestions assistant.
     */
    public List<Integer> findTopSellingProductIds(int limit) throws SQLException {
        if (limit <= 0) {
            return List.of();
        }
        String sql = "SELECT li.produit_id AS pid, COALESCE(SUM(li.quantite), 0) AS sold "
    ...
    private Product map(ResultSet rs) throws SQLException {
        ...
        String stockNom = rs.getString("stock_nom");
        p.setStockNom(rs.wasNull() ? null : stockNom);
