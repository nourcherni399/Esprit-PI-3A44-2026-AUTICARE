...
public class UserService implements IService<User> {
    ...
    private static final String MYSQL_USER_SELECT =
            "SELECT id, nom, prenom, email, CAST(telephone AS CHAR) AS telephone, password AS mot_de_passe_hash, "
                    ...
                    + "date_naissance, adresse, tarif_consultation, sexe, created_at, updated_at, image, "
                    + "google_calendar_refresh_token FROM `user` WHERE ";
    ...
    public void update(User u) throws SQLException {
        String sql = "UPDATE `user` SET nom=?, prenom=?, email=?, telephone=?, password=?, is_active=?, role=?, type=?, "
                ...
                + "tarif_consultation=?, google_calendar_refresh_token=?, updated_at=? WHERE id=?";
        try (PreparedStatement ps = MyDatabase.getConnection().prepareStatement(sql)) {
    ...
    public List<User> findAll() throws SQLException {
        ...
        String sql = "SELECT id, nom, prenom, email, CAST(telephone AS CHAR) AS telephone, password AS mot_de_passe_hash, "
                + "is_active AS actif, role, specialite, nom_cabinet AS cabinet, relation_avec_patient AS relation_parent, "
                + "date_naissance, adresse, tarif_consultation, sexe, created_at, updated_at, image, "
                + "google_calendar_refresh_token FROM `user` ORDER BY created_at DESC";
    ...
    public List<User> search(String keyword) throws SQLException {
        ...
        String sql = "SELECT id, nom, prenom, email, CAST(telephone AS CHAR) AS telephone, password AS mot_de_passe_hash, "
                + "is_active AS actif, role, specialite, nom_cabinet AS cabinet, relation_avec_patient AS relation_parent, "
                + "date_naissance, adresse, tarif_consultation, sexe, created_at, updated_at, image, "
                + "google_calendar_refresh_token FROM `user` "
    ...
    public List<User> findByRole(Role role) throws SQLException {
        ...
        String sql = "SELECT id, nom, prenom, email, CAST(telephone AS CHAR) AS telephone, password AS mot_de_passe_hash, "
                + "is_active AS actif, role, specialite, nom_cabinet AS cabinet, relation_avec_patient AS relation_parent, "
                + "date_naissance, adresse, tarif_consultation, sexe, created_at, updated_at, image, "
                + "google_calendar_refresh_token FROM `user` WHERE role=? ORDER BY created_at DESC";
        try (PreparedStatement ps = MyDatabase.getConnection().prepareStatement(sql)) {
    ...
    private void fillMysqlUpdate(PreparedStatement ps, User u) throws SQLException {
        ...
        ps.setString(17, u.getGoogleCalendarRefreshToken());
    ...
    private User map(ResultSet rs) throws SQLException {
        ...
        u.setGoogleCalendarRefreshToken(rs.getString("google_calendar_refresh_token"));
        return u;
    }

    /** Met à jour (ou efface) le jeton OAuth Google Agenda pour l’API Calendar. */
    public void updateGoogleCalendarRefreshToken(int userId, String refreshTokenOrNull) throws SQLException {
        ...
        try (PreparedStatement ps = MyDatabase.getConnection().prepareStatement(sql)) {
            if (refreshTokenOrNull != null && !refreshTokenOrNull.isBlank()) {
                ps.setString(1, refreshTokenOrNull);
            } else {
