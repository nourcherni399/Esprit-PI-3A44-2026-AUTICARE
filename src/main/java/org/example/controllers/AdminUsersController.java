...
public class AdminUsersController {
    ...
    private void reloadUsers() {
        try {
            ...
            alert(Alert.AlertType.ERROR, "Erreur", e.getMessage() != null ? e.getMessage() : "Erreur base de données.");
        }
    }
    ...
    private void alertError(String title, IOException e) {
        alert(Alert.AlertType.ERROR, title, formatIoExceptionMessage(e));
    }

    /** Affiche la cause utile (ex. ressource FXML manquante) au lieu du chemin tronqué du LoadException. */
    private static String formatIoExceptionMessage(IOException e) {
        if (e instanceof LoadException le) {
            Throwable nested = le.getCause();
            if (nested != null) {
                String m = nested.getMessage();
                if (m != null && !m.isBlank()) {
                    return m;
                }
                return nested.getClass().getSimpleName();
            }
        }
