...
public class PublicShellController {
    ...
    private void refreshTopNavState() {
        ...
        boolean showPatientBell = logged && u != null
                && (u.getRole() == Role.PATIENT || u.getRole() == Role.PARENT);
        if (patientNotifBellHost != null) {
            patientNotifBellHost.setVisible(showPatientBell);
            patientNotifBellHost.setManaged(showPatientBell);
        }
        if (logged) {
    ...
    /**
     * Pages « compactes » : pas de bandeau hero fixe ni de spacer de 320 px dans le ScrollPane
     * (évite un grand vide + scroll inutile sur connexion, inscription et parcours RDV).
     */
    private void applyShellHeroForPage(String pageId) {
        boolean compact = usesCompactShellLayout(pageId);
        boolean rdvPage = isRdvPageId(pageId);

        /*
         * Le fond photo ne doit pas rester visible sur toutes les pages publiques.
         * On l'affiche uniquement pour le parcours RDV, sinon on masque les calques image.
         */
        if (globalBgA != null) {
        ...
        if (shellHeroLayer != null) {
            shellHeroLayer.setVisible(!compact);
            shellHeroLayer.setManaged(!compact);
        }
    ...
    private static boolean usesCompactShellLayout(String pageId) {
        if (isRdvPageId(pageId)) {
            return true;
        }
