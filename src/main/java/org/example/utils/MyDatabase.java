...
public class MyDatabase {
    ...
    public Connection openConnection() throws SQLException {
        if (connection == null || connection.isClosed()) {
            try {
                ...
                ensureRendezVousGestionTokenColumn(connection);
                ensureRendezVousSmsRappel24hEnvoyeAtColumn(connection);
    ...
    /**
     * Colonne {@code date_heure} en DATETIME : les dumps Symfony n’ont souvent que {@code date_rdv} (DATE),
     * ce qui force l’heure à 00:00 côté JDBC. On ajoute {@code date_heure} et on le remplit depuis {@code disponibilite}.
     */
    private static void ensureRendezVousDateHeureDatetimeColumn(Connection c) {
        try (Statement st = c.createStatement()) {
            try {
                st.execute("ALTER TABLE rendez_vous ADD COLUMN date_heure DATETIME NULL DEFAULT NULL");
            } catch (SQLException e) {
                if (!isDuplicateColumnError(e)) {
                    System.err.println("Migration rendez_vous.date_heure : " + e.getMessage());
                }
            }
            try {
                st.execute(
                ...
                System.err.println("Migration backfill rendez_vous.date_heure (disponibilite) : " + e.getMessage());
            }
            try {
                st.execute(
                        "UPDATE rendez_vous SET date_heure = CAST(date_rdv AS DATETIME) "
                                + "WHERE date_heure IS NULL AND date_rdv IS NOT NULL");
            } catch (SQLException e) {
                if (!isMissingColumnMessage(e, "date_rdv")) {
                    System.err.println("Migration backfill rendez_vous.date_heure (date_rdv) : " + e.getMessage());
                }
            }
        } catch (SQLException e) {
            System.err.println("Migration date_heure : " + e.getMessage());
        }
    }

    private static boolean isMissingColumnMessage(SQLException e, String columnName) {
        String m = e.getMessage();
        if (m == null) {
            return false;
        }
        String lower = m.toLowerCase();
        return lower.contains("unknown column") && lower.contains(columnName.toLowerCase(Locale.ROOT));
    }

    /** Jeton OAuth refresh pour l’API Google Calendar (médecin). */
    private static void ensureUserGoogleCalendarRefreshTokenColumn(Connection c) {
        try (Statement st = c.createStatement()) {
            try {
                st.execute("ALTER TABLE `user` ADD COLUMN google_calendar_refresh_token TEXT NULL DEFAULT NULL");
            } catch (SQLException e) {
                if (!isDuplicateColumnError(e)) {
    ...
    /** Horodatage envoi SMS rappel ~24 h avant le RDV (Twilio). */
    private static void ensureRendezVousSmsRappel24hEnvoyeAtColumn(Connection c) {
        try (Statement st = c.createStatement()) {
            try {
                st.execute("ALTER TABLE rendez_vous ADD COLUMN sms_rappel_24h_envoye_at DATETIME NULL DEFAULT NULL");
            } catch (SQLException e) {
                if (!isDuplicateColumnError(e)) {
                    System.err.println("Migration rendez_vous.sms_rappel_24h_envoye_at : " + e.getMessage());
                }
            }
        } catch (SQLException e) {
            System.err.println("Migration sms_rappel_24h_envoye_at : " + e.getMessage());
        }
    }

    /** Jeton pour lien « gérer le RDV » dans les e-mails patients. */
    private static void ensureRendezVousGestionTokenColumn(Connection c) {
        try (Statement st = c.createStatement()) {
            try {
                st.execute("ALTER TABLE rendez_vous ADD COLUMN gestion_token VARCHAR(64) NULL DEFAULT NULL");
            } catch (SQLException e) {
                if (!isDuplicateColumnError(e)) {
                    System.err.println("Migration rendez_vous.gestion_token : " + e.getMessage());
                }
            }
        } catch (SQLException e) {
            System.err.println("Migration gestion_token : " + e.getMessage());
        }
    }
